...
namespace client
{
    class StartCreateSingleVmExample : Scenario
    {
        ...
        private async System.Threading.Tasks.Task ExcuteAsync()
        {
            ...
            var subnet = (await(vnet.GetSubnetContainer().Construct("10.0.0.0/24").StartCreate(Context.SubnetName)).WaitForCompletionAsync()).Value;
            ...
            var vm = (await(resourceGroup.GetVirtualMachineContainer().Construct(Context.Hostname, "admin-user", "!@#$%asdfA", nic.Id, aset.Data).StartCreate(Context.VmName)).WaitForCompletionAsync()).Value;
